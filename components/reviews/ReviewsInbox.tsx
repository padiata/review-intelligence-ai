...
export default function ReviewsInbox() {
  ...
  useEffect(() => {
    async function loadInbox() {
      ...
      if (reviewsResult.error) {
        ...
        setReviews(
          (reviewsResult.data as ImportedReview[] | null) ?? []
        );
      }
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter((review) => {
      ...
      const matchesSource =
        sourceFilter === "all" ||
        review.source.toUpperCase() === sourceFilter.toUpperCase();

      const matchesStatus =
        statusFilter === "all" ||
        (statusFilter === "open"
          ? ["new", "pending", "in_review"].includes(
              review.analysis_status
            )
          : review.analysis_status === statusFilter);

      return matchesSearch && matchesSource && matchesStatus;
    });
  }, [reviews, search, sourceFilter, statusFilter]);

  function getSourceName(sourceCode: string) {
    return (
      sources.find(
        (source) =>
  ...
  return (
    <div className="reviews-inbox">
      <section className="reviews-inbox-header">
        <div>
          <p className="eyebrow">Reviews recibidas</p>
          <h2>Bandeja de reviews</h2>
          <span>
      ...
      <section className="reviews-filters">
        ...
        >
          <option value="open">Pendientes de gestión</option>
      ...
      <section className="reviews-list">
        {filteredReviews.map((review) => {
          const score = normalizeRating(review.rating);
            ...
            >
              <div className="review-list-content">
                <div className="review-list-meta">
                  ...
                  <span>
                    {statusLabel(review.analysis_status)}
                  </span>
                  ...
                  {review.priority && (
                    <span>Prioridad {review.priority}</span>
                  )}
                </div>
                ...
                <p>
                  {(review.review_text ??
                    "La review no contiene texto.").slice(0, 220)}
